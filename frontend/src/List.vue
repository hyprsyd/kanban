    ...
    <div class="list-group" style="margin: 10px">
      <nav class="navbar">
        ...
        <div class="list-title">
          <h1 style="color: #ff669d;font-family: 'JetBrains Mono ExtraBold' ,sans-serif;margin:1px 20px 10px; text-align: center">{{ list.title }}</h1>
        </div>
        <button class="btn btn-dark" @click="edit(list)">Edit</button>
      </nav>
      <div class="cards">
        <card v-for="card in cardsByList" :key="card.id" :cardId="card.id" :card="card" :listId="listId" :description="card.description" :title="card.title"></card>
      </div>
      <div class="add-card-form">
        <form @submit.prevent="addCard(listId)">
          ...
          <input class="input-group-text" style="height: 80px;" v-model="newCardDescription" type="text" placeholder="Add a new card description"/>
